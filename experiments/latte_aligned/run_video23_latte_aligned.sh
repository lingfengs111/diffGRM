...
sid="$repo/cache/AmazonReviews2023CleanGR/Video_Games/processed/sentence-t5-base_latte-meta_pca192_RQKMEANS3x256_psid.sem_ids"
ar_run=video23_latte_rqkmeans3_latent8_ar_l20_v1
...
run_ar() {
  CUDA_VISIBLE_DEVICES="$gpu" TOKENIZERS_PARALLELISM=false "$python_bin" main.py \
    ...
    --config=experiments/latte_aligned/video23_rqkmeans3_latent8.yaml \
...
run_onepass() {
  ...
  CUDA_VISIBLE_DEVICES="$gpu" TOKENIZERS_PARALLELISM=false "$python_bin" \
    ...
    --sid-config experiments/latte_aligned/video23_rqkmeans3_latent8.yaml \
