...
while tmux has-session -t video23_half_ar_drafter_0831 2>/dev/null; do
  sleep 30
done
cd "$repo"
...
env TOKENIZERS_PARALLELISM=false "$python_bin" \
  ...
  --ar-checkpoint saved/AmazonReviews2023CleanGR_video23_full_opq_cf_ar_half_2x2_d176_v1/pytorch_model.bin \
  --init-trained-checkpoint "$run_root/one_pass_pairwise_ar_fusion/best.pt" \
  ...
  --output-dir "$run_root/one_pass_pairwise_ar_fusion_converged" \
  >"$run_root/one_pass_pairwise_ar_fusion_converged.log" 2>&1
