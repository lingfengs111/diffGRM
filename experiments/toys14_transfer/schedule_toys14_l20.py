#!/usr/bin/env python3
"""Run the full-data Toys14 L20 comparison whenever a GPU becomes free."""
...
ROOT = REPO / 'runs/toys14_transfer/queue'
DONE = ROOT / 'done'
...
JOBS = [
    {
        'name': 'toys14_ar', 'priority': 10,
        'command': 'bash experiments/toys14_transfer/run_toys14_ar.sh {gpu}',
        'requires': [], 'depends': [],
        'outputs': [REPO / 'saved/AmazonReviews2014CleanGR_toys14_full_opq_cf_ar_l20_long_v1/pytorch_model.bin'],
    },
    {
        'name': 'toys14_sasrec', 'priority': 20,
        'command': 'bash /home/lingfengs111/codes/GR/CleanGR/scripts/run_amazon14_toys_sasrec_l20.sh {gpu}',
        'requires': [], 'depends': [],
        'outputs': [HOME / 'codes/GR/CleanGR/outputs/amazon14_toys/sasrec_long_l20/eval/test_metrics.json'],
    },
    {
        'name': 'toys14_rpg', 'priority': 30,
        'command': 'bash /home/lingfengs111/codes/GR_variant/RPG_KDD2025/experiments/run_toys14_sentence_t5_l20.sh {gpu}',
        'requires': [], 'depends': [], 'outputs': [],
    },
    {
        'name': 'toys14_diffgrm', 'priority': 40,
        'command': 'bash experiments/toys14_transfer/run_toys14_diffgrm.sh {gpu}',
        'requires': [
        ...
        'depends': [],
        'outputs': [REPO / 'saved/AmazonReviews2014CleanGR_toys14_full_opq_cf_diff_guided_l20_long_v1/pytorch_model.bin'],
    },
    {
        'name': 'toys14_onepass', 'priority': 50,
        'command': 'bash experiments/toys14_transfer/run_toys14_onepass.sh {gpu}',
        'requires': [], 'depends': ['toys14_ar'],
        'outputs': [REPO / 'runs/toys14_transfer/random_encoder4_pairwise_ar/converged/result.json'],
    },
]
...
def main():
    ...
    while True:
        ...
        if all(state['jobs'].get(j['name'], {}).get('status') in {'done', 'failed'} for j in JOBS):
            save(state)
            print('Toys14 L20 queue finished', flush=True)
            return
