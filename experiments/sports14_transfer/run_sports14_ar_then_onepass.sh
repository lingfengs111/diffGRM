...
ar_run=sports14_full_opq_cf_ar_l20_long_v1
...
# The AR job owns first-time Sentence-T5/PCA/OPQ4 construction. Hungarian
# repair makes the four-coordinate catalog injective before any decoder sees it.
if [[ ! -s "$ar_ckpt" ]]; then
...
# From-scratch one-pass structured drafter, matching the strongest Video23
# recipe: a high-LR first stage followed by validation-stopped continuation.
CUDA_VISIBLE_DEVICES="$gpu" TOKENIZERS_PARALLELISM=false "$python_bin" \
  ...
  --sid-config experiments/amazon14_domains/sports14.yaml \
